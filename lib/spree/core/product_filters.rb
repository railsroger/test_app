  ...
  module Core
    ...
    module ProductFilters
      ...
      Spree::Property.find_each do |property|
        Spree::Product.add_search_scope :"#{property.name}" do |*opts|
          conds = opts.map { |o| ProductFilters.send(:"#{property.name}_filter")[:conds][o] }.reject(&:nil?)
          scope = conds.shift
          conds.each do |new_scope|
            scope = scope.or(new_scope)
          end
          Spree::Product.with_property(property.name).where(scope)
        end
        (class << ProductFilters; self end).class_eval do
          define_method(:"#{property.name}_filter") do
            values = property ? Spree::ProductProperty.where(property_id: property.id).pluck(:value).uniq.map(&:to_s) : []
            if property.is_number?
              values.sort! { |x, y| x.to_f <=> y.to_f }
            end
            pp = Spree::ProductProperty.arel_table
