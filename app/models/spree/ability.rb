...
module Spree
  class Ability
    ...
    def initialize(user)
      ...
      if user.respond_to?(:has_spree_role?) && user.has_spree_role?('super')
        can :manage, :all
      elsif user.respond_to?(:has_spree_role?) && user.has_spree_role?('admin')
        can :manage, :all
        cannot [:update, :delete], User, spree_roles: { name: 'super' }
      elsif user.respond_to?(:has_spree_role?) && user.has_spree_role?('content_manager')
        can :manage, :all
        cannot :manage, User
        cannot :manage, Store
      else
      ...
      cannot [:update, :destroy], Role, name: ['super']
    end
